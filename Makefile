...
ubuntu:
	@ sudo apt-get -y install zlibc curl libssl0.9.8 libbz2-dev libxslt1-dev libxml2-dev libsqlite3-0 
	# 2>&1 >> tracking.log
...
test-coverage-travis-ci:
	@ $(SOURCE_ACTIVATE) coverage run --source='stations/' tests/stations_test.py
...
clean:
	...
	@ rm -rf MANIFEST virtualenv* sqlite* bin/ lib/ lib64 include/ build/ share setuptools-*.tar.gz get-pip.py tracking.log subversion .Python

hardclean:
	@ echo "[ cleaning     ] remove the sqlite3 and hdf5 C libraries from /usr/local/lib"
